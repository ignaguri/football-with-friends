...
app.post("/copy-from/:sourceGroupId", async (c) => {
  const denied = requireOrganizer(c);
  if (denied) return denied;
  ...
  if (sourceGroupId === current.id) {
    return c.json({ error: "Source and target group must differ" }, 400);
  }

  if (!isPlatformAdmin(c)) {
    const sourceMembership = await getRepositoryFactory().groupMembers.find(sourceGroupId, user.id);
    if (!sourceMembership || sourceMembership.role !== "organizer") {
      return c.json({ error: "Must be an organizer of the source group" }, 403);
    }
  } else {
    ...
    const sourceGroup = await getRepositoryFactory().groups.findById(sourceGroupId);
    if (!sourceGroup) {
  ...
  try {
    const result = await getServiceFactory().groupService.copyVenues(sourceGroupId, current.id);

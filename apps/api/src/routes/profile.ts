...
const updateProfileSchema = z.object({
  ...
  nationality: z
    ...
    .optional()
    .nullable(),
  phoneNumber: z.string().regex(phoneRegex, "Invalid phone number format").optional().nullable(),
  ...
  try {
    ...
    if (!bucket) {
      ...
      await db
        .updateTable("user")
        .set({
          profilePicture: `https://ui-avatars.com/api/?name=${encodeURIComponent(userId)}&size=200`,
        })
        .where("id", "=", userId)
        .execute();
...
app.post("/update-profile", zValidator("json", updateProfileSchema), async (c) => {
  ...
  const userId = requireUser(c).id;
  const { username, displayUsername, nationality, phoneNumber, email, name } = data;
  ...
  try {
    ...
    if (displayUsername !== undefined) updateData.displayUsername = displayUsername || null;
    ...
    if (Object.keys(updateData).length > 0) {
      await db.updateTable("user").set(updateData).where("id", "=", userId).execute();
    }
...
app.post("/update-username", async (c) => {
  ...
  if (nationality && nationality !== null) {
    if (!/^[A-Z]{2}$/.test(nationality)) {
      return c.json({ error: "Nationality must be a valid 2-letter ISO country code" }, 400);
    }
  }
...
app.post("/set-password", zValidator("json", setPasswordSchema), async (c) => {
  ...
  try {
    ...
    if (credentialAccount) {
      return c.json(
        {
          error:
            "You already have a password set. Please use the Change Password feature with your current password.",
    ...
    if (
      errorMessage.includes("already has a password") ||
...
app.post("/delete-account", zValidator("json", deleteAccountSchema), async (c) => {

...
Today the app is a single global namespace. Every logged-in user sees every match, every venue, every player; the only authorization dimension is a global `user.role ∈ {"user","admin"}`. This works for one friend group but blocks every next step — a second organizer can't coexist, production data leaks between cohorts, and there's no notion of "this match is in *my* realm."
...
| **Naming** | What to call it | **`group`** (table `groups`, context `currentGroup`, invite URLs `/invite/<token>`) | Neutral, matches WhatsApp/Slack/Discord mental model. i18n-friendly (*grupo* in ES). |
